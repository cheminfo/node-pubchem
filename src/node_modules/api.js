...
const moleculesEm = co.wrap(function* moleculesEm(query) {
    ...
    const data = yield dataCollection;
    const result = yield data.find(mongoQuery).project({_id: 1, iupac: 1, ocl: 1, mf: 1}).limit(10000).toArray();
...
const moleculesMf = co.wrap(function* moleculesMf(query = {}) {
    ...
    const data = yield dataCollection;
    const result = yield data.find({mf: query.mf}).project({_id: 1, iupac: 1, ocl: 1, mf: 1}).limit(10000).toArray();
    return result.map(el => {
        return {
            id: el._id,
            iupac: el.iupac,

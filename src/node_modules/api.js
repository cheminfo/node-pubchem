...
const moleculesEm = co.wrap(function* moleculesEm(value) {
    ...
    const data = yield dataCollection;
    const result = yield data.find({em: value}).project({_id: 1, iupac: 1, ocl: 1}).limit(10000).toArray();
...
const moleculesMf = co.wrap(function* moleculesMf(value) {
    ...
    const data = yield dataCollection;
    const result = yield data.find({mf: value}).project({_id: 1, iupac: 1, ocl: 1}).limit(10000).toArray();
    return result.map(el => {
        return {
            id: el._id,
            iupac: el.iupac,

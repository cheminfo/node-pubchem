...
const moleculesEm = co.wrap(function* moleculesEm(value) {
    if (typeof value !== 'number') {
        throw new TypeError('value must be a number');
    }

    const data = yield dataCollection;
    const result = yield data.find({em: value}).project({_id: 1, iupac: 1, ocl: 1, mf: 1}).limit(10000).toArray();
...
const moleculesMf = co.wrap(function* moleculesMf(value) {
    if (typeof value !== 'string') {
        throw new TypeError('value must be a string');
    }

    const data = yield dataCollection;
    const result = yield data.find({mf: value}).project({_id: 1, iupac: 1, ocl: 1, mf: 1}).limit(10000).toArray();
    return result.map(el => {
        return {
            id: el._id,
            iupac: el.iupac,
